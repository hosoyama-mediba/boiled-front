...
-- Table `front`.`cp`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `front`.`list` ;

CREATE TABLE IF NOT EXISTS `front`.`list` (
...
CREATE TABLE IF NOT EXISTS `front`.`todo` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `title` VARCHAR(255) NOT NULL COMMENT 'title',
  `desc` VARCHAR(255) NOT NULL COMMENT 'desc',
  `isComplete` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'isComplete',
  `created_date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '作成日',
  `updated_date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
